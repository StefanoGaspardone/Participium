...
export default function Map({ selected, setSelected }: Props) {
  ...
  const nearestPointOnTurin = useCallback(
    ...
    fetch(nominatimUrl)
      ...
      .then((results: NominatimResult[]) => {
        ...
        if (!feature || !feature.geojson)
          throw new Error("No polygon found for Turin");
        ...
        if (gj && gj.type === "Polygon") {
          ...
          turinPolysRef.current = [
            outer.map((pair: [number, number]) =>
              leaflet.latLng(pair[0], pair[1])
            ),
          ];
