...
import { getReportsByStatus, getCategories, updateReportCategory, acceptOrRejectReport } from '../api/api';
...
export default function PROHomepage() {
    ...
    const acceptReport = async (report: PendingReport) => {
        try {
            ...
            await acceptOrRejectReport(report.id, 'Assigned');
    ...
    const rejectReport = async (report: PendingReport) => {
        ...
        try {
            setStatusUpdatingId(report.id);
            await acceptOrRejectReport(report.id, 'Rejected', reason);
            setReports(rs => rs.filter(r => r.id !== report.id));
        } catch (e: any) {
