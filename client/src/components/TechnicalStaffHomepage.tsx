...
export default function TechnicalStaffHomepage() {
  ...
  const [assigningReportId, setAssigningReportId] = useState<number | null>(
    null
  );
  const [maintainersByReportId, setMaintainersByReportId] = useState<User[]>(
    []
  );
  const [selectedMaintainerByReportId, setSelectedMaintainerByReportId] =
  ...
  const [imageIndexByReport, setImageIndexByReport] = useState<
    Record<number, number>
  >({});
  const [addressByReport, setAddressByReport] = useState<
  ...
  // open chat when navigated from notification
  const loc = useLocation();
  type NavState = {
    openChat?: { report?: Partial<Report>; targetUserId?: number | null };
  };
  useEffect(() => {
    const payload = (loc.state as NavState)?.openChat;
    if (!payload) return;
    ...
    setChatTargetUserId(payload.targetUserId ?? null);
    setShow(true);
  ...
  const handleRToggle = (id: number) => {
    ...
    if (next !== null && !addressByReport[id]) {
      ...
      if (report)
        fetchAndSetAddress(report.id, Number(report.lat), Number(report.long));
    }
  };
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setError(
          e instanceof Error ? e.message : "Failed to load assigned reports"
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleStatusChange = async (
    reportId: number,
    newStatus: "InProgress" | "Suspended" | "Resolved"
  ) => {
    ...
    try {
      ...
      setError(
        e instanceof Error ? e.message : "Failed to update report status"
      );
    } finally {
  ...
  const handleAssignMaintainer = async (
    reportId: number,
    overrideMaintainerId?: number
  ) => {
    const maintainerId =
      overrideMaintainerId ?? selectedMaintainerByReportId[reportId];
    if (!maintainerId) return;
    try {
      setAssigningReportId(reportId);
      const report = await assignReportToExternalMaintainer(
        reportId,
        maintainerId
      );
        ...
        <div className="container py-5">
          ...
          {user?.offices && user.offices.length > 0 && (
            ...
            >
              <motion.h3
                ...
                transition={{ duration: 0.5 }}
              >
                {user.offices.length === 1 ? "My office" : "My offices"}
              </motion.h3>
                ...
                {user.offices.map((office) => (
                  <Badge
                    ...
                    style={{
                      ...
                      background:
                        "linear-gradient(90deg, #265ea8 0%, #1e4a7f 100%)",
                      fontWeight: 500,
                      borderRadius: "8px",
          ...
          {loading && (
            ...
            >
              ...
              <span
                className="ms-2"
                style={{ color: "#265ea8", fontWeight: 500 }}
              >
                Loading reports...
              </span>
            </motion.div>
              ...
              {reports.map((r, idx) => (
                ...
                >
                  <Accordion.Item eventKey={String(r.id)} className="mt-4">
                    <Accordion.Header onClick={() => handleRToggle(r.id)}>
                      <button
                        type="button"
                        onClick={() => handleFetchMaintainers(r)}
                        onKeyDown={(e) => {
                          if (e.key === "Enter" || e.key === " ") {
                            handleFetchMaintainers(r);
                          }
                        }}
                        ...
                        >
                          <span
                            ...
                            style={{
                              ...
                              marginRight: "0.5rem",
                              backgroundColor:
                                REPORT_STATUS_COLORS[r.status] || "#64748B",
                              color: "white",
                    ...
                    <Accordion.Body>
                              ...
                              >
                                {r.images.length > 1 && (
                                  <motion.button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      showPrevImage(r.id, r.images.length);
                                    }}
                                ...
                                {r.images.length > 1 && (
                                  <motion.button
                                    ...
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      showNextImage(r.id, r.images.length);
                                    }}
                                ...
                                <div className="image-counter">
                                  {getCurrentImageIndex(r.id, r.images.length) +
                                    1}{" "}
                                  / {r.images.length}
                                </div>
                              </div>
                      ...
                      <div className="row">
                        <div className="col-6">
                          <div className="mb-3">
                            <h5 style={{ color: "#00205B", fontWeight: 600 }}>
                              Category
                            </h5>
                            <Badge
                              bg="secondary"
                            ...
                            {getAvailableActions(r.status).length > 0 ? (
                                ...
                                {getAvailableActions(r.status).map((action) => {
                                  ...
                                  return (
                                    <Button
                                      ...
                                      onClick={() =>
                                        handleStatusChange(r.id, action.value)
                                      }
                              ...
                              >
                                <Alert variant="info" className="mb-0 py-2">
                                  This report is completed and cannot be
                                  modified.
                                </Alert>
                              </motion.div>
                            )}
                        ...
                        <div className="col-6">
                          <div className="mb-3">
                            <h5 style={{ color: "#00205B", fontWeight: 600 }}>
                              Assign to an external maintainer
                            </h5>
                            {r?.coAssignedTo ? (
                              r?.coAssignedTo.id === 13 ? (
                                <p className="">
                                  Assigned to external maintainer out of
                                  Participium
                                </p>
                              ) : (
                                <div className="d-flex align-items-center gap-2">
                                  ...
                                  <div>
                                    <div style={{ fontWeight: 600 }}>
                                      {r.coAssignedTo.firstName}{" "}
                                      {r.coAssignedTo.lastName}
                                    </div>
                                    <div style={{ fontWeight: 700 }}>
                                      {r.coAssignedTo.company?.name.toUpperCase() ??
                                        "COMPANY"}
                                    </div>
                                  </div>
                                </div>
                              ...
                              <div>
                                <div className="d-flex align-items-center gap-2 mb-1">
                                  <select
                                    ...
                                    value={
                                      selectedMaintainerByReportId[r.id] ?? ""
                                    }
                                    onChange={(e) =>
                                      setSelectedMaintainerByReportId(
                                        (prev) => ({
                                          ...prev,
                                ...
                                >
                                  ...
                                  <button
                                    ...
                                    onClick={() =>
                                      handleAssignMaintainer(r.id, 13)
                                    }
                          ...
                          <div>
                            ...
                            >
                              <button
                                ...
                                onClick={() => {
                                  setChatTargetUserId(r.createdBy?.id ?? null);
                                  setActiveReport(r);
                                  setShow(true);
                                }}
                                className="auth-link-inline"
